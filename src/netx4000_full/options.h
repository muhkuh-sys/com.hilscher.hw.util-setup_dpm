...
typedef struct STRUCT_SYSTEM_CONFIG
{
	...
	unsigned long  ulBootBlockOldstyleMagic;
	unsigned char  ucBootImage_ForceAlternative; /* The RUN jumper is set. This will set ucBootImage_Standard0_Alternative1 to 1 later. */
	unsigned char  ucBootImage_Standard0_Alternative1; /* This selects the image type for the current boot process. */
	unsigned char  ucBootImage_Prevent_Switch_to_Alternative; /* Prevent switching to the alternative loader if no valid image was found. */
	unsigned char  ucForceSecureConsole;
	unsigned char auc_mmio_sel[NUMBER_OF_MMIOS];
	unsigned char auc_mmio_inv[NUMBER_OF_MMIOS];
} SYSTEM_CONFIG_T;


typedef struct PORT_CONTROL_DEFAULTS_STRUCT
{
	unsigned short aus[291];
} PORT_CONTROL_DEFAULTS_T;
...
typedef struct STRUCT_ROMLOADER_OPTIONS
{
	...
	PORT_CONTROL_DEFAULTS_T tPortControlDefault;
} ROMLOADER_OPTIONS_T;
