...
static void dpm_deinit_registers(void) {
	...
	sizCnt = sizeof(ptHandshakeCtrl0Area->aulHandshake_hsc_ctrl)
	...
	sizCnt = sizeof(ptHandshakeCtrl0Area->asHandshake_buf_man)
			/ sizeof(ptHandshakeCtrl0Area->asHandshake_buf_man[0]);
	do {
		--sizCnt;
		ptHandshakeCtrl0Area->asHandshake_buf_man[sizCnt].ulCtrl = HOSTDFLT(
				handshake_buf_man0_ctrl);
		ptHandshakeCtrl0Area->asHandshake_buf_man[sizCnt].ulStatus_ctrl_netx =
				HOSTDFLT(handshake_buf_man0_status_ctrl_netx);
...
static void dpm_init(DPM_TRANSPORT_TYPE_T tDpmTransportType, HIF_CONFIG_T* ptDpmConfig) {
	...
	if (ptDpmConfig != NULL) {
		...
		ptDpmArea->ulDpm_addr_cfg = ptDpmConfig->tDpmConfig.ulDpmAddrCfg;
		ptDpmArea->ulDpm_timing_cfg = ptDpmConfig->tDpmConfig.ulDpmTimingCfg;
		ptDpmArea->ulDpm_rdy_cfg = ptDpmConfig->tDpmConfig.ulDpmRdyCfg;
		ptDpmArea->ulDpm_misc_cfg = ptDpmConfig->tDpmConfig.ulDpmMiscCfg;
...
static BOOTING_T pcie_init(int idpm) {
	...
	ulResetSource &=
			HOSTMSK(
					RAP_SYSCTRL_RSTSTAT_SWRST_STAT) | HOSTMSK(RAP_SYSCTRL_RSTSTAT_STAT_NETX_DPM);
	...
	if (ulResetSource != 0 && ulValue != 0) {
		...
		cr7_global_timer_start_ticks(&tHandle,
				g_t_romloader_options.tPcieOptions.ulPowerUpTimeoutTicks);
		do {
			/* Reached the timeout? */
		...
		ulValue |=
				((unsigned long) (g_t_romloader_options.tPcieOptions.ucAcpCachePolicyPcie))
						<< HOSTSRT(RAP_SYSCTRL_NOCAXICFG_PCIE);
		...
		if (tResult == BOOTING_Ok) {
			...
			cr7_global_timer_delay_ticks(
					g_t_romloader_options.tPcieOptions.ulResetDelayTicks);
			...
			/* Set the device ID. */
			ptPCIEArea->ulPCIE_PCI_CR_ID =
					g_t_romloader_options.tPcieOptions.ulVidPid;
			ptPCIEArea->ulPCIE_PCI_CR_CLASS_REV =
					g_t_romloader_options.tPcieOptions.ulRevisionClassCode;
			...
			case 0:
				...
				break;
			case 1:
				ptPCIEArea->ulPCIE_ADEST0 = HOSTADDR(intramhs1_dpm_mirror);
				break;
			default: break;
			}
...
//		trace_message(TRACEMSG_DPM_DpmClocksMaskedOut);
		...
		/* Initialize the DPM. */
		dpm_init(tDpmTransportType, ptDpmConfig);
		tResult = BOOTING_Ok;
	}
